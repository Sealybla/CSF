...
int lru(int set_index, map<int, vector<CacheBlock>> &cache, int num_block, int &total_cycles, CacheBlock &new_block, int pcycles_per_block, int access_time) {
    vector<CacheBlock> &all_blocks = cache[set_index]; 
    //finding min total_cycles (most nonused)
    int min = all_blocks[0].last_used; 
    int min_idx = 0; 
   ...
   //loading to cache takes 1 cycle
    new_block.last_used = access_time;
    all_blocks[min_idx] = new_block; 
    return min_idx; 
}

//same as the lru 
int fifo(int set_index, map<int, vector<CacheBlock>> &cache, int num_block, int &total_cycles, CacheBlock &new_block, int pcycles_per_block, int access_time) {
    vector<CacheBlock> &all_blocks = cache[set_index]; 
    //finding min total_cycles (most nonused)
    int min = all_blocks[0].last_used; 
    int min_idx = 0; 
...
int main( int argc, char **argv ) {
  ...
  if (write_policy == "write-back" && write_allocate_policy == "no-write-allocate") {
    std::cerr << "Error: policy cannot be both 'no-write-allocate' and 'write-back'." << std::endl;
    return 1;
  }
  ...
  while (cin>>operation>>std::hex>>address_str>>bytes) {
    ...
    for (CacheBlock &block : cache[set_index]) {
      if (block.valid && block.tag == tag) {
        ...
        if (eviction_policy == "lru") {
          block.last_used = access_time; //update last used time
        }
        break;
      }
    }
    if (!hit) {
      ...
      if (operation == 'l') {
        ...
        new_block.last_used = access_time;
        new_block.load_time = access_time;
        if (cache[set_index].size() < num_block) {
          ...
          if (eviction_policy == "lru") {
            lru(set_index, cache, num_block, total_cycles, new_block, pcycles_per_block, access_time);
          }
          else {//if (eviction_policy == "fifo")
            fifo(set_index, cache, num_block, total_cycles, new_block, pcycles_per_block, access_time);
          }
        }
      } else {//operation == 's'
        store_misses ++; 
        if (write_allocate_policy == "write-allocate") {
          ...
          if (cache[set_index].size() < num_block) { //space available in set
            ...
            if (eviction_policy == "lru") {
              vector<CacheBlock> &all_blocks = cache[set_index];
              int min_idx = lru(set_index, cache, num_block, total_cycles, new_block, pcycles_per_block, access_time);
              ...
              vector<CacheBlock> &all_blocks = cache[set_index];
              int min_idx = fifo(set_index, cache, num_block, total_cycles, new_block, pcycles_per_block, access_time);
              if(write_policy == "write-through") {
                total_cycles += 100;
                total_cycles += 1;;  //store with write-through after miss
              } else {
